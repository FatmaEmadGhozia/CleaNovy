...
export default function ContactPage() {
  ...
  const inputCls =
    "w-full px-4 py-3 rounded-xl border border-gray-200 bg-white text-right text-sm text-[#0D1F3C] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#00C9B1] focus:border-transparent transition-all";
      ...
      <section className="max-w-7xl mx-auto px-6 lg:px-12 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-gray-50 rounded-3xl p-8">
            ...
            {error && (
              <p className="text-[#FF4D4D] text-sm mb-4 text-right">{error}</p>
            )}
            ...
            <button
              ...
              disabled={loading}
              className="cp-submit flex items-center gap-2 px-6 py-3 rounded-2xl font-semibold text-white transition-all duration-200 hover:opacity-90 hover:-translate-y-0.5 shadow-md disabled:opacity-60"
            >
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-gray-50 rounded-3xl p-6">
              ...
              <div className="flex items-center justify-between gap-3 p-4 bg-white rounded-2xl mb-3 hover:shadow-sm transition-shadow cursor-pointer">
                <div className="cp-icon-bg w-10 h-10 rounded-full flex items-center justify-center shrink-0">
                  <MessageCircle className="h-5 w-5 text-[#00C9B1]" />
                </div>
              ...
              <div className="flex items-center justify-between gap-3 p-4 bg-white rounded-2xl hover:shadow-sm transition-shadow cursor-pointer">
                <div className="cp-icon-bg w-10 h-10 rounded-full flex items-center justify-center shrink-0">
                  <Phone className="h-5 w-5 text-[#00C9B1]" />
                </div>
                <div className="text-right flex-1">
                  <p className="text-sm font-semibold text-[#0D1F3C]">اتصال هاتفي</p>
                  <p className="text-xs text-[#0D1F3C]/50">
            ...
            <div className="rounded-3xl overflow-hidden relative h-44">
              ...
              />
              <div className="absolute inset-0 bg-linear-to-t from-[#0D1F3C]/70 to-transparent" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mt-12">
          {[
            { icon: <ShieldCheck className="h-7 w-7 text-[#00C9B1]" />, title: "أمن وسري",   desc: "بياناتك دائماً في أمان"      },
            { icon: <Zap         className="h-7 w-7 text-[#00C9B1]" />, title: "رد سريع",    desc: "خلال أقل من 24 ساعة"         },
            { icon: <Headphones  className="h-7 w-7 text-[#00C9B1]" />, title: "دعم مستمر",  desc: "نساعدك خطوة بخطوة"           },
          ].map((item, i) => (
            <div key={i} className="text-center py-6">
              <div className="cp-feat-bg w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-3">
                {item.icon}
              </div>
