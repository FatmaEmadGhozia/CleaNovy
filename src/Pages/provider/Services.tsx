...
//     const newId = await addServiceCategory(newCatLabel.trim(), newCatIcon);
//     if (newId) setActiveCategory(newId);
//     setNewCatLabel("");
...
//   const openEdit = (svc: Service, e: React.MouseEvent) => {
...
//     if (!editSvc) return;
//     await updateService(editSvc.id, editForm);
//     setEditSvc(null);
//   };
...
//                       <span className="material-symbols-outlined" style={activeCategory === cat.id ? { fontVariationSettings: "'FILL' 1" } : {}}>
//                         {cat.icon}
//                       </span>
//                       {cat.label}
//                     </span>
...
//                     className={`p-5 flex items-center gap-6 transition-colors cursor-pointer ${
//                       svc.active ? "hover:bg-[#EFF4FA]/40" : "bg-slate-50/50 grayscale opacity-60"
//                     }`}
//                   >
...
//                                 if (val && val !== svc.price) updateService(svc.id, { price: val });
//                               }}
//                             />
//                           ) : (
//                             <span>{svc.price}</span>
...
//                           }`}
//                         >
//                           <span className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${svc.fastService ? "left-1" : "right-1"}`} />
//                         </button>
//                       </div>
...
//                             {svc.multiplier}
//                           </span>
//                           {svc.active && <span className="material-symbols-outlined text-xs text-slate-400">edit</span>}
//                         </div>
//                       </div>

//                       <div className="flex flex-col items-center gap-1">
//                         <p className="text-xs text-slate-400">الحالة</p>
//                         <button onClick={() => updateService(svc.id, { active: !svc.active })} className="transition-all">
//                           {svc.active ? (
//                             <div className="flex items-center gap-2 bg-[#006B5D]/10 text-[#006B5D] px-3 py-1 rounded-full text-xs font-bold hover:bg-red-50 hover:text-red-500 transition-colors">
//                               <span className="w-1.5 h-1.5 bg-[#006B5D] rounded-full animate-pulse"></span>
//                               متاح
//                             </div>
//                           ) : (
//                             <div className="flex items-center gap-2 bg-slate-200 text-slate-500 px-3 py-1 rounded-full text-xs font-bold hover:bg-[#006B5D]/10 hover:text-[#006B5D] transition-colors">
//                               متوقف
//                             </div>
//                           )}
//                         </button>
...
//       {editSvc && (
//         <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
//           <div className="absolute inset-0 bg-[#0D1F3C]/60 backdrop-blur-sm" onClick={() => setEditSvc(null)} />
//           <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6">
//             <div className="flex justify-between items-center mb-6">
//               <h3 className="text-lg font-bold text-[#0D1F3C]" style={{ fontFamily: "Cairo, sans-serif" }}>تعديل الخدمة</h3>
//               <button onClick={() => setEditSvc(null)} className="text-slate-400 hover:text-[#0D1F3C]">
//                 <span className="material-symbols-outlined">close</span>
//               </button>
//             </div>
...
//                       className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
//                         editForm.icon === icon ? "bg-[#006B5D] text-white" : "bg-[#EFF4FA] text-[#0D1F3C]"
//                       }`}
//                     >
//                       <span className="material-symbols-outlined text-lg">{icon}</span>
//                     </button>
...
//               {/* Description */}
//               <div>
//                 <label className="block text-sm font-bold text-[#0D1F3C] mb-2">الوصف</label>
//                 <textarea
//                   value={editForm.description ?? ""}
//                   onChange={(e) => setEditForm((f) => ({ ...f, description: e.target.value }))}
//                   rows={2}
//                   className="w-full bg-[#EFF4FA] rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-[#006B5D]/20 text-sm resize-none"
//                 />
//               </div>
...
//                 <div>
//                   <label className="block text-sm font-bold text-[#0D1F3C] mb-2">السعر (ر.س)</label>
//                   <input
...
//                   <select
//                     value={editForm.multiplier ?? "x1.5"}
//                     onChange={(e) => setEditForm((f) => ({ ...f, multiplier: e.target.value }))}
//                     disabled={!editForm.fastService}
//                     className="w-full bg-[#EFF4FA] rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-[#006B5D]/20 text-sm disabled:opacity-50"
//                   >
//                     {MULTIPLIER_OPTIONS.map((m) => <option key={m} value={m}>{m}</option>)}
//                   </select>
//                 </div>
//               </div>
...
//                     <button
//                       key={d.value}
//                       onClick={() => setEditForm((f) => ({ ...f, durationHours: d.value }))}
//                       className={`px-3 py-2 rounded-xl text-sm font-bold transition-all ${
//                         editForm.durationHours === d.value ? "bg-[#006B5D] text-white" : "bg-[#EFF4FA] text-[#0D1F3C]"
//                       }`}
//                     >
//                       {d.label}
//                     </button>
...
//               {/* Fast service + Active toggles */}
//               <div className="flex gap-4">
//                 <div className="flex-1 flex items-center justify-between p-4 bg-[#EFF4FA] rounded-2xl">
//                   <div>
//                     <p className="font-bold text-sm text-[#0D1F3C]">الخدمة السريعة</p>
//                   </div>
//                   <button
//                     onClick={() => setEditForm((f) => ({ ...f, fastService: !f.fastService }))}
//                     className={`w-12 h-6 rounded-full relative transition-all shadow-inner ${editForm.fastService ? "bg-[#00C9B1]" : "bg-slate-300"}`}
//                   >
//                     <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${editForm.fastService ? "left-1" : "right-1"}`} />
//                   </button>
//                 </div>
//                 <div className="flex-1 flex items-center justify-between p-4 bg-[#EFF4FA] rounded-2xl">
//                   <div>
...
//                     className={`w-12 h-6 rounded-full relative transition-all shadow-inner ${editForm.active ? "bg-[#00C9B1]" : "bg-slate-300"}`}
//                   >
//                     <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${editForm.active ? "left-1" : "right-1"}`} />
//                   </button>
//                 </div>
//               </div>
...
//                   className="flex-1 py-3 bg-[#EFF4FA] text-[#0D1F3C] rounded-full font-bold text-sm hover:bg-slate-200 transition-all"
//                 >
//                   إلغاء
//                 </button>
//                 <button
//                   onClick={handleEditSave}
//                   className="flex-1 py-3 bg-[#006B5D] text-white rounded-full font-bold text-sm hover:bg-[#005046] transition-all"
//                 >
//                   حفظ التعديلات
//                 </button>
//               </div>
...
export default function Services() {
  ...
  const handleEditSave = async () => {
    ...
    // compute duration string from durationHours
    const durationHours = editForm.durationHours ?? editSvc.durationHours;
    const duration = durationHours < 24
      ? `الوقت المتوقع: ${durationHours} ساعة`
      : `الوقت المتوقع: ${Math.floor(durationHours / 24)} ${Math.floor(durationHours / 24) === 1 ? "يوم" : "أيام"}`;

    const finalForm = { ...editForm, duration };

    setEditSvc(null); // close modal immediately — optimistic
    await updateService(editSvc.id, finalForm); // API in background
  };
      ...
      <div className="flex gap-8">
        ...
        <aside className="w-64 flex flex-col gap-4 flex-shrink-0">
          <div className="p-4 bg-white rounded-2xl shadow-sm border border-[#0D1F3C]/5">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowAddCategory(true)} className="p-1.5 rounded-lg text-[#006B5D] hover:bg-[#EFF4FA] transition-all">
                <span className="material-symbols-outlined text-lg">add_circle</span>
              </button>
            </div>
            <div className="space-y-1">
              {categories.map((cat) => (
                <div key={cat.id} className="group flex items-center gap-1">
                  <button
                    onClick={() => setActiveCategory(cat.id)}
                    className={`flex-1 flex items-center justify-between px-3 py-3 rounded-xl transition-all ${
                      activeCategory === cat.id ? "bg-[#EFF4FA] text-[#006B5D] font-bold" : "text-slate-500 hover:bg-[#EFF4FA]/50 hover:text-[#0D1F3C]"
                    }`}
                  >
                    <span className="flex items-center gap-3">
                      <span className="material-symbols-outlined" style={activeCategory === cat.id ? { fontVariationSettings: "'FILL' 1" } : {}}>{cat.icon}</span>
                      {cat.label}
                    </span>
                    <span className={`text-xs ${activeCategory === cat.id ? "bg-white px-2 py-0.5 rounded-full shadow-sm" : ""}`}>{cat.count}</span>
                  </button>
                  <button
                    onClick={() => handleDeleteCategory(cat.id)}
                    className="p-1.5 rounded-lg text-slate-300 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-all"
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-[#0D1F3C] to-[#006B5D] text-white shadow-xl overflow-hidden relative group">
            <div className="relative z-10">
              <p className="text-xs text-[#00C9B1] font-bold mb-1">تلميحة العمليات</p>
              <h4 className="text-sm leading-snug" style={{ fontFamily: "Cairo, sans-serif" }}>تفعيل "الخدمة السريعة" يزيد من دخل الطلب الواحد بنسبة تصل لـ 40%</h4>
            </div>
            <span className="material-symbols-outlined absolute -bottom-4 -left-4 text-white/10 text-8xl rotate-12 group-hover:rotate-0 transition-transform duration-500">trending_up</span>
          </div>
        </aside>
        ...
        <div className="flex-1">
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-[#0D1F3C]/5">
            <div className="p-6 border-b border-[#E4E9EF] flex justify-between items-center bg-[#EFF4FA]/30">
              <div className="flex items-center gap-4">
                <span className="font-bold text-[#0D1F3C]" style={{ fontFamily: "Cairo, sans-serif" }}>{activeCategoryData?.label ?? "الخدمات"}</span>
                <div className="h-4 w-px bg-slate-300"></div>
                <span className="text-sm text-slate-500">عرض {activeCount} خدمة مفعّلة من {filteredServices.length}</span>
              </div>
              <div className="flex gap-2">
                <button className="p-2 rounded-lg bg-white border border-slate-200 text-slate-400 hover:text-[#0D1F3C] transition-all">
                  <span className="material-symbols-outlined">sort</span>
            ...
            <div className="divide-y divide-[#E4E9EF]">
              {filteredServices.length === 0 ? (
                ...
                filteredServices.map((svc) => (
                  ...
                  >
                    <div className="text-slate-300"><span className="material-symbols-outlined">drag_indicator</span></div>

                    <div className={`w-14 h-14 rounded-xl flex items-center justify-center ${svc.active ? "bg-[#E9EEF4]" : "bg-slate-200"}`}>
                      <span className={`material-symbols-outlined text-3xl ${svc.active ? "text-[#0D1F3C]" : "text-slate-400"}`}>{svc.icon}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-10">
                      {/* Price — value not defaultValue so it updates instantly */}
                      <div className="text-center">
                        <p className="text-xs text-slate-400 mb-1">سعر الوحدة</p>
                        <div className="flex items-center gap-1 font-bold text-[#0D1F3C]">
                          <input
                      ...
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <button
                          onClick={() => updateService(svc.id, { fastService: !svc.fastService })}
                          className={`w-10 h-5 rounded-full relative transition-all shadow-inner ${svc.fastService ? "bg-[#00C9B1]" : "bg-slate-300"}`}
                        >
                          <span className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${svc.fastService ? "left-1" : "right-1"}`} />
                        </button>
                      </div>
                      ...
                      <div className="text-center">
                        <p className="text-xs text-slate-400 mb-1">مضاعف السعر</p>
                        <div className={`flex items-center gap-1 rounded-lg px-2 py-1 ${svc.active ? "bg-[#E9EEF4]" : "bg-slate-100"}`}>
                          <span className={`text-xs font-bold ${svc.active ? "text-[#765B00]" : "text-slate-400"}`}>{svc.multiplier}</span>
                          {svc.active && <span className="material-symbols-outlined text-xs text-slate-400">edit</span>}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <button onClick={() => updateService(svc.id, { active: !svc.active })} className="transition-all">
                          {svc.active ? (
                            <div className="flex items-center gap-2 bg-[#006B5D]/10 text-[#006B5D] px-3 py-1 rounded-full text-xs font-bold hover:bg-red-50 hover:text-red-500 transition-colors">
                              <span className="w-1.5 h-1.5 bg-[#006B5D] rounded-full animate-pulse"></span>متاح
                            </div>
                          ) : (
                            <div className="flex items-center gap-2 bg-slate-200 text-slate-500 px-3 py-1 rounded-full text-xs font-bold hover:bg-[#006B5D]/10 hover:text-[#006B5D] transition-colors">متوقف</div>
                          )}
                        </button>
                      </div>

                      <button onClick={() => deleteService(svc.id)} className="p-2 text-slate-300 hover:text-red-600 transition-colors">
            ...
            <div className="p-8 border-t border-dashed border-slate-200 flex flex-col items-center justify-center text-center">
              ...
              <h5 className="font-bold text-[#0D1F3C]" style={{ fontFamily: "Cairo, sans-serif" }}>أضف خدمة فرعية جديدة لهذا التصنيف</h5>
              <p className="text-sm text-slate-400 mt-1 max-w-xs">يمكنك تحديد الأسعار، مدة التنفيذ، وإضافة خيارات الخدمة السريعة فوراً</p>
              <button onClick={() => setAddServiceOpen(true)} className="mt-4 px-6 py-2 bg-[#EFF4FA] text-[#006B5D] rounded-full text-sm font-bold hover:bg-[#006B5D] hover:text-white transition-all">
                بدء الإضافة
              </button>
            </div>
          </div>
      ...
      <div className="mt-8 grid grid-cols-3 gap-6">
        {[
          ...
          <div key={s.label} className="bg-white p-6 rounded-2xl shadow-sm border border-[#0D1F3C]/5 flex items-center gap-5">
            ...
            <div>
              <p className="text-sm text-slate-500">{s.label}</p>
              <p className="text-2xl font-bold text-[#0D1F3C]">{s.value} <span className="text-xs font-normal text-slate-400">{s.sub}</span></p>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-bold text-[#0D1F3C] mb-2">اسم التصنيف</label>
                <input value={newCatLabel} onChange={(e) => setNewCatLabel(e.target.value)} placeholder="مثال: أزياء رسمية" className="w-full bg-[#EFF4FA] rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-[#006B5D]/20 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-bold text-[#0D1F3C] mb-2">الأيقونة</label>
                <div className="flex flex-wrap gap-2">
                  {SERVICE_ICONS.map((icon) => (
                    <button key={icon} onClick={() => setNewCatIcon(icon)} className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${newCatIcon === icon ? "bg-[#006B5D] text-white" : "bg-[#EFF4FA] text-[#0D1F3C]"}`}>
                      <span className="material-symbols-outlined text-lg">{icon}</span>
                    </button>
                  ))}
                </div>
              </div>
              <button onClick={handleAddCategory} className="w-full py-3 bg-[#006B5D] text-white rounded-full font-bold text-sm hover:bg-[#005046]">إضافة التصنيف</button>
            </div>
          </div>
        </div>
      )}

      {/* Edit Service Modal — wide not tall */}
      {editSvc && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-[#0D1F3C]/60 backdrop-blur-sm" onClick={() => setEditSvc(null)} />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl p-8">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-[#0D1F3C]" style={{ fontFamily: "Cairo, sans-serif" }}>تعديل الخدمة</h3>
              <button onClick={() => setEditSvc(null)} className="text-slate-400 hover:text-[#0D1F3C]">
                <span className="material-symbols-outlined">close</span>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SERVICE_ICONS.map((icon) => (
                      <button key={icon} onClick={() => setEditForm((f) => ({ ...f, icon }))}
                        className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${editForm.icon === icon ? "bg-[#006B5D] text-white" : "bg-[#EFF4FA] text-[#0D1F3C]"}`}>
                        <span className="material-symbols-outlined text-lg">{icon}</span>
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <textarea value={editForm.description ?? ""} onChange={(e) => setEditForm((f) => ({ ...f, description: e.target.value }))}
                    rows={3} className="w-full bg-[#EFF4FA] rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-[#006B5D]/20 text-sm resize-none" />
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input value={editForm.price ?? ""} onChange={(e) => setEditForm((f) => ({ ...f, price: e.target.value }))}
                      type="text" className="w-full bg-[#EFF4FA] rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-[#006B5D]/20 text-sm" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-[#0D1F3C] mb-2">مضاعف السعر السريع</label>
                    <select value={editForm.multiplier ?? "x1.5"} onChange={(e) => setEditForm((f) => ({ ...f, multiplier: e.target.value }))}
                      disabled={!editForm.fastService}
                      className="w-full bg-[#EFF4FA] rounded-xl py-3 px-4 outline-none focus:ring-2 focus:ring-[#006B5D]/20 text-sm disabled:opacity-50">
                      {MULTIPLIER_OPTIONS.map((m) => <option key={m} value={m}>{m}</option>)}
                    </select>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {DURATION_OPTIONS.map((d) => (
                      <button key={d.value} onClick={() => setEditForm((f) => ({ ...f, durationHours: d.value }))}
                        className={`px-3 py-2 rounded-xl text-sm font-bold transition-all ${editForm.durationHours === d.value ? "bg-[#006B5D] text-white" : "bg-[#EFF4FA] text-[#0D1F3C]"}`}>
                        {d.label}
                      </button>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex items-center justify-between p-4 bg-[#EFF4FA] rounded-2xl">
                    <p className="font-bold text-sm text-[#0D1F3C]">الخدمة السريعة</p>
                    <button onClick={() => setEditForm((f) => ({ ...f, fastService: !f.fastService }))}
                      className={`w-12 h-6 rounded-full relative transition-all shadow-inner ${editForm.fastService ? "bg-[#00C9B1]" : "bg-slate-300"}`}>
                      <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all shadow-sm ${editForm.fastService ? "left-1" : "right-1"}`} />
                    </button>
                  </div>
                ...
                {/* Actions */}
                <div className="flex gap-3 pt-2">
                  <button onClick={() => setEditSvc(null)} className="flex-1 py-3 bg-[#EFF4FA] text-[#0D1F3C] rounded-full font-bold text-sm hover:bg-slate-200 transition-all">إلغاء</button>
                  <button onClick={handleEditSave} className="flex-1 py-3 bg-[#006B5D] text-white rounded-full font-bold text-sm hover:bg-[#005046] transition-all">حفظ التعديلات</button>
                </div>
              </div>
            </div>
          </div>
